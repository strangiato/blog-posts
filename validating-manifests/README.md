...
When making changes to objects directly on the cluster the user is immediately presented with feedback when issues exist and the user is able to troubleshoot and resolve those issues with the knowledge of the context of the changes they just made.  When working with GitOps, that feedback cycle is often delayed and users don't receive feedback on changes until they are applied to the cluster, which could be hours or even weeks depending on the approval lifecycle of a change.

To help reduce the number of errors in changes to a GitOps manifest illiminate the dreaded `Unknown` state in an ArgoCD Application, this article will discuss tools and best practices that can help to reduce the feedback cycle on those changes.  We will discuss automating these validations with GitHub actions, but all of these validations can be configured with another CI tool of your choice.

## YAML Linting

YAML is basis of nearly all GitOps repos.  As you would expect, YAML has specific syntax standards for what is valid and what is not.  Additionally, there are many recommended best practices that may not be required but can help improve the consistency and readability of YAML objects.
...
Many different YAML Linter tools exists, but one that I prefer is [yamllint](https://github.com/adrienverge/yamllint).  `yamllint` is built with Python, making it easy to setup on most Linux/MacOS environment since Python is installed by default and Python can be easily installed on any Windows environment.
...
Running `yamllint` locally is a great option for developers to get feedback while making changes to a repo, however running `yamllint` directly in a CI tool such as GitHub Actions can help to enforce standards and prevent improperly formatted YAML from ever making it into the main branch.  Thankfully a pre-built GitHub actions make it easy to add this validation.

To add a yamllint GitHub action, simply create a file called `.github/workflows/validate-manifests.yaml` containing the following:
...
This GitHub Action has direct integration with GitHub and can do annotations directly on the lines of code with issues, making it easier for developers to identify problems and resolve them.
...
However, YAML linters are generally not able to do any sort of deeper inspection of the objects and they do not have any knowledge if the object is valid or not.
...
```sh
kustomize build <path-to-kustomize-folder>
```
...
Kustomize is an incredibly powerful tool, but one that is easy for human error to cause problems.  This simple practice of validating every `kustomization.yaml` file in a repo can help to reduce the number of errors created by accidentally misspelling a filename, or forgetting to update a filename in the `kustomization.yaml` file after renaming it.  This kustomize check can also help to identify problems where objects are updated and any patches that impact those objects are no longer valid.

Additional, this validation can help to ensure that you don't accidentally break dependencies where another `kustomization.yaml` file inherit from a folder you did make changes to.  Problems such as when an object is removed from a base folder and that same object is being patched in the overlay can be quickly caught before the changes are ever merged into the main branch.

## Validating Helm

Helm is another popular tool that is utilized in GitOps repos for organizing complex applications.  Helm is an extremely powerful tool that can users can easily introduce errors 
